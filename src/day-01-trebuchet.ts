...
const stringsArr: string[] = input.split("\n");

const calibrationValueP1 = () => {
  const digitsOnly = stringsArr.map((str) => {
    const digitsRegex = str.match(/\d/g);
    const firstAndLastDigit = digitsRegex
      ? digitsRegex[0] + digitsRegex.slice(-1)
      : 0;
    return +firstAndLastDigit;
  });
  return digitsOnly.reduce((acc, cur) => acc + cur, 0);
};

const calibrationValueP2 = () => {
  const modifiedSpelledNumbers = stringsArr.map((str) => {
    const regex = /zero|one|two|three|four|five|six|seven|eight|nine/gi;
    const spelledNumberMap: Record<string, string> = {
      zero: "zzeroo",
      one: "oonee",
      two: "ttwoo",
      three: "tthree",
    ...
    return str.replace(regex, (match) => spelledNumberMap[match]);
  });

  const arraysOfDigitsAndSpelledDigits = modifiedSpelledNumbers.map((str) => {
    const spelledDigitsRegex =
      /(one|two|three|four|five|six|seven|eight|nine|\d)/g;
    const result = str.match(spelledDigitsRegex);
    return result;
  });

  const convertedDigits = arraysOfDigitsAndSpelledDigits.map((arr) => {
    if (arr === null) return 0;
    const digitsArr = arr.map((value) => {
      const spelledDigitsArray = [
        "zero",
        "one",
        "two",
        "three",
      ...
      const digitIsSpelled = spelledDigitsArray.some(
        (spelled) => spelled === value
      );
      return digitIsSpelled
        ? spelledDigitsArray.indexOf(value).toString()
        : value;
    });

    const firstAndLastDigit = digitsArr[0] + digitsArr[digitsArr.length - 1];

    return +firstAndLastDigit;
  });

  return convertedDigits.reduce((acc, cur) => acc + cur, 0);
};

console.log(calibrationValueP2());
